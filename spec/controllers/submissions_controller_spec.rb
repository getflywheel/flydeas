...
RSpec.describe SubmissionsController, type: :controller do
  ...
  describe "get #create" do
    it "allows for a submission to be created" do 
      get :create, :submission => {title: @sub.title, content: @sub.content, user_id: @user.id, 
                      category_id: @sub.category_id} 
      expect(response).to have_http_status(302)
    end
    it "doesnt allow for subission to be created (invalid params)" do
      #@invalid_sub =  create(:invalid_submission)
      get :create, :submission => {title: nil, content: nil, user_id: nil, 
                      category_id: nil}
  ...
  describe "Put #update" do
    it "allows update to content" do
      ...
      get :update, { :submission => { content: content, user_id: @sub.user_id,
                     category_id: @sub.category_id }, id: @sub.id }
      @sub.reload
    ...
    it "denies update when not logged in" do
      logout                  
      content = @sub.content
      get :update,  { :submission => { content: "new stuff", user_id: @sub.user_id,
                     category_id: @sub.category_id }, id: @sub.id } 
      @sub.reload
      expect(@sub.content).to include(content)            
    end 
